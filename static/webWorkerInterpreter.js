...
var Exec = function(stmts) {
  ...
  var evalExp = function(e, env) {
    ...
    if (e === 'executeOrders' || e[0] === 'executeOrders') {
      ...
      */
      self.postMessage({'type': 'executeOrders', 'ordersJson':{id : ordersToPost['id'], orders: orderString}})
      sleep(100)
      ordersToPost['orders'] = []
      return true
    }
  ...
  var evalStmt = function(stmts, env){
    ...
    for (i in stmts){
      ...
      if (s[0] === 'exp'){
        ...
        self.postMessage("PRINTING")
        self.postMessage(evalExp(s[1],env))
